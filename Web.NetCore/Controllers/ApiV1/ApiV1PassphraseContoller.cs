...
namespace MurrayGrant.MakeMeAPassword.Web.NetCore.Controllers.ApiV1
{
    ...
    public class ApiV1PassphraseContoller : ApiV1Controller
    {
        ...
        public ApiV1PassphraseContoller(PooledEntropyCprngGenerator terninger, PasswordRatingService ratingService, PasswordStatisticService statisticService, IpThrottlerService ipThrottler, DictionaryService dictionaryService) 
            : base(terninger, ratingService, statisticService, ipThrottler, dictionaryService) { }

        [HttpGet("/api/v1/passphrase/plain")]
        public async Task<IActionResult> Plain([FromQuery]int? pc, [FromQuery]int? wc, [FromQuery]string sp, [FromQuery]int? minCh, [FromQuery]int? maxCh, [FromQuery]NumericStyles? whenNum, [FromQuery]int? nums, [FromQuery]AllUppercaseStyles? whenUp, [FromQuery]int? ups)
        {
            // Return as plain text string.
            using (var random = await _Terninger.CreateCypherBasedGeneratorAsync())
            {
                var phraseDictionary = await _DictionaryService.ReadPassphraseDictionary();
                var phrases = SelectPhrases(phraseDictionary, random,
        ...
        public async Task<IActionResult> Json([FromQuery]int? pc, [FromQuery]int? wc, [FromQuery]string sp, [FromQuery]int? minCh, [FromQuery]int? maxCh, [FromQuery]NumericStyles? whenNum, [FromQuery]int? nums, [FromQuery]AllUppercaseStyles? whenUp, [FromQuery]int? ups)
        {
        ...
        public async Task<IActionResult> Xml([FromQuery]int? pc, [FromQuery]int? wc, [FromQuery]string sp, [FromQuery]int? minCh, [FromQuery]int? maxCh, [FromQuery]NumericStyles? whenNum, [FromQuery]int? nums, [FromQuery]AllUppercaseStyles? whenUp, [FromQuery]int? ups)
        {
            // Return as XML.
            using (var random = await _Terninger.CreateCypherBasedGeneratorAsync())
            {
