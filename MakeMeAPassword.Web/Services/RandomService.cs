...
namespace MurrayGrant.PasswordGenerator.Web.Services
{
    ...
    public sealed class RandomService
    {
        ...
        public byte[] GetNextBytes(int length)
        {
            ...
            this.LoadNextBytes(result);
            return result;
        }
        public void LoadNextBytes(byte[] bytes)
        {
            if (bytes == null)
        ...
        private void LoadNextBytesInternal(byte[] bytes)
        {
            ...
            {
                var block = GetRandomBlock(bytes.Length - idx);
                Buffer.BlockCopy(block, 0, bytes, idx, block.Length);
        ...
        public int Next()
        {
            this.LoadNextBytes(_FourByteBuffer);
